    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      <div>
        ...
        <p className="text-slate-500">
          Here's a summary of your tutoring practice this month.
        </p>
      </div>
        ...
        {statCards.map((stat, i) => (
          ...
          >
            <CardContent className="p-5">
              ...
              <h3
                className={`text-2xl font-bold ${stat.title === "Pending Dues" ? "text-rose-500" : "text-slate-800"}`}
              >
                {stat.value}
              </h3>
              <p
                className={`text-xs font-medium mt-2 ${
                  stat.title === "Monthly Earnings"
                    ? "text-indigo-500"
                    : stat.title === "Total Students"
                      ? "text-emerald-500"
                      : stat.title === "Pending Dues"
                        ? "text-slate-400"
                        : "text-amber-500"
                }`}
              >
            ...
            <Table>
              <TableHeader>
                <TableRow className="bg-slate-50 hover:bg-slate-50">
                  <TableHead className="px-6 py-3 text-[10px] font-bold text-slate-400 uppercase">
                    Student
                  </TableHead>
                  <TableHead className="px-6 py-3 text-[10px] font-bold text-slate-400 uppercase">
                    Amount
                  </TableHead>
                  <TableHead className="px-6 py-3 text-[10px] font-bold text-slate-400 uppercase text-right">
                    Action
                  </TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {recentPayments.map((p) => (
                  ...
                  >
                    ...
                    <TableCell className="px-6 py-4 text-sm font-bold text-rose-500">
                      ${p.amount}
                    </TableCell>
                    <TableCell className="px-6 py-4 text-right">
                      <Button
                        variant="secondary"
                        size="sm"
                        className="bg-indigo-50 text-indigo-600 hover:bg-indigo-100 font-bold text-xs"
                      >
                        Record
                      </Button>
                    </TableCell>
